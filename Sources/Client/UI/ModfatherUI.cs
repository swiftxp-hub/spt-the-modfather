...
public static class ModfatherUI
{
    ...
    public static void Draw(
        string statusText,
        bool showButtons,
        bool showProgress,         // VEREINHEITLICHT: Ersetzt isChecking + isUpdating
        bool isError,
        float progressValue,
        ...
        Action onErrorContinue)
    {
        ...
        try
        {
            ...
            DrawStatusText(winRect, statusText);
            ...
            if (isError)
            {
                // Roter Balken, Progress bleibt beim letzten Stand stehen
                DrawProgressBar(barRect, progressValue, progressDetail, new Color(0.7f, 0.0f, 0.0f, 1f));
            ...
            else if (showProgress)
            {
                ...
                DrawProgressBar(barRect, progressValue, progressDetail, ModfatherUIColors.AccentOrange);
            ...
            // C) LISTE & AUSWAHL
            else if (showButtons && actions != null)
            {
                ...
                DrawActionList(listRect, actions);
    ...
    private static void DrawListHeader(Rect rect, IReadOnlyList<SyncAction> items)
    {
        ...
        float bW = 90f;
        if (GUI.Button(new Rect(rect.x + rect.width - bW - 5, rect.y + 2, bW, 22), "Select None")) items.ForEach(x => x.IsSelected = false);
        if (GUI.Button(new Rect(rect.x + rect.width - (bW * 2) - 10, rect.y + 2, bW, 22), "Select All")) items.ForEach(x => x.IsSelected = true);
    }
